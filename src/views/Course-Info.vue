        ...
        <div class="info">
          ...
          <ul class="teacherName">
            <li class="name-item">
              ...
              <img src="../../public/image/teacherStart.png" alt="" />
            </li>
            <li class="name-item">金牌讲师</li>
          </ul>
          <ul class="access">
            <li class="access-item">难度</li>
            <li class="access-item">{{level(courseInfo.courseLevel)}}</li>
...
import { getCourseDetail,courseCheckAuth,downloadAttachment } from "../common/api/course";
...
const downloadSource = (item)=>{
  ...
  courseCheckAuth({
    ...
    downloadAttachment({
      ...
      const blob = new Blob([res]);//获取bolb流
      let fileName = item.attachmentName;//获取文件名
      let fileUrl = item.attachmentUrl;//获取url
      ...
      link.href = URL.createObjectURL(blob);//创建url对象,链进来
      document.body.appendChild(link);
      link.click();
      URL.revokeObjectURL(link.href);//删除url对象
      document.body.removeChild(link);//删除a节点
    })
  })
}
//立即播放
...
.courseInfoTop {
  width: 100%;
  height: 200px;
  background-image: url("../../public/image/courseInfobg1920.png");
}
.nav-container {
  width: 1200px;
  margin: 0 auto;
