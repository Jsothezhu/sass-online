      ...
      <div class="login-box">
        ...
        <div class="login-left">
          <a href="/" title="小鹿线" class="logo">
            <img src="../../public/image/logo02.png" alt="小鹿线" title="小鹿线">
          </a>
                ...
                <div class="tab-main">
                  ...
                  >
                    <el-form-item class="login-user" prop="phone">
                      <el-icon><avatar /></el-icon>
                      <el-input placeholder="请输入您的手机号"  v-model="ruleFormPhone.phone"/>
                    </el-form-item>

                    <el-form-item class="login-Verification" prop="captcha">
                      <el-input placeholder="请输入您的验证码" v-model="ruleFormPhone.captcha"/>
                      <el-button class="btn btn-primary sendcaptcha" type="primary" @click="showVerify">{{captcha}}</el-button>
                    </el-form-item>
...
import { login,sendCaptcha ,loginByMobile } from "../common/api/login";
  ...
  formEl.validate((valid, fields) => {
    if (valid) {
      login({
        ...
        if(data.meta.code === '10006'){
          //获取pinia的userstore,必须在方法内部使用，否则有可能获取不到
...
const showVerify = ()=>{
  ...
  let phone = ruleFormPhone.phone;
  if( regTel.test( phone )  ){  //校验成功,展示验证码
    verify.value.show();
  }
}
//点击短信登录
...
.container{
  ...
  width: 100vw;
  height: 100vh;
  background: url("../../public/image/loginbg.jpeg") no-repeat center;
}
section{
  position: absolute;
  top: 0;
...
.login-right{
  ...
  width: 475px;
  height: 500px;
  background: url("../../public/image/ybbg.jpeg") no-repeat center center;
}
