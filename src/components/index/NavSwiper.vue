  ...
  <div class='navSwiper'>
    <div class='content'>
      <div class='navigation'>
        <ul>
          <li v-for="(item,index) in firstCategorys" :key="item.id" @mouseenter="mouseHover(item.id,index)"
          @mouseleave="mouseLeave(index)">
            <router-link to='/'>
              {{item.categoryName}}
              <el-icon color='#ffffff' :size='16'><arrow-right /></el-icon>
            </router-link>
            <div class='category-detail' ref="detail">
              <div class='detail-main'>
                <div class='detail-desc'>基础知识</div>
                <div class='detail-list'>
                  <div class='list-know'>知识点：</div>
                  <div class='list-ul'>
                    <router-link to='/' class='list-item'>{{item.description}}</router-link>
                  </div>
                </div>
                <div class='detail-class' v-for="(item1) in secondCategorys" :key="item1.id">
                  <div class='course-card' >
                    ...
                    <div class='right'>
                      ...
                      <div class="courseDegree">{{ level(item1.level) }} · 584人购买</div>
                      <div class='buy'>
                        <div class='buy-free'>
      ...
      <div class='sliders'>
        <el-carousel :interval="5000" arrow="always"  height="460px" v-if="swiperList.length !== 0">
    ...
    <div class='course-type'>
      <div class='course-type-item'>
        <router-link to='/'>
          <div class='course-type-item-icon'>
            <img src="../../../public/image/chuji.png">
          </div>
      ...
      <div class='course-type-item'>
        <router-link to='/'>
          <div class='course-type-item-icon'>
            <img src="../../../public/image/zhongji.png">
          </div>
          <div class='course-type-item-text'>
      ...
      <div class='course-type-item'>
        <router-link to='/'>
          <div class='course-type-item-icon'>
            <img src="../../../public/image/gaoji.png">
          </div>
          <div class='course-type-item-text'>
      ...
      <div class='course-type-item'>
        <router-link to='/'>
          <div class='course-type-item-icon'>
            <img src="../../../public/image/xiangmu.png">
          </div>
          <div class='course-type-item-text'>
      ...
      <div class='course-type-item'>
        <router-link to='/'>
          <div class='course-type-item-icon'>
            <img src="../../../public/image/suanfa.png">
          </div>
          <div class='course-type-item-text'>
...
import { swiperImg,getFirstCategorys,getSecondCategorys } from "../../common/api/index";
import {ElMessage} from "element-plus";
import { level } from "../../utils/mixin";
  const detail = ref(null);
  const secondCategorys = ref([]);//二级课程
  const mouseHover = (categoryId,index) => {
    detail.value[index].style.display = "block"
    getSecondCategorys(categoryId)
        .then((data)=>{
            secondCategorys.value = data.data.list;
         })
        .catch((error)=>{
          ElMessage({
            message: error,
  ...
  const mouseLeave = (index)=>{
    detail.value[index].style.display = "none";
    secondCategorys.value = [];
  }
  let swiperList = ref([]);
  const firstCategorys = ref([]);
    onBeforeMount(()=>{
      ...
      swiperImg()
          .then((data)=>{
            if(data.meta.code === "200"){
              swiperList.value = data.data.list;
            }
          })
          .catch((error)=>{
            ElMessage({
      ...
      getFirstCategorys()
          .then((data)=>{
            if(data.meta.code === "200"){
              firstCategorys.value = data.data.list;
            }
          })
          .catch((error)=>{
            ElMessage({
